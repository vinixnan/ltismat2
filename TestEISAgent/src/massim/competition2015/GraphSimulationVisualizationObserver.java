...
import massim.competition2015.GraphPolicy.EdgeDummy;
...
public class GraphSimulationVisualizationObserver implements Observer {
    ...
    private final int fastModeCounterIncrease = 5;
    private int fastModeCounterInitial = 0;
    private int fastModeCounter = 0;
    //private long renderingTimeLimitNs = 1000 * 1000000;
    private long zoneRenderingTimeoutMs = 1500;
    ...
    public void notifySimulationConfiguration(SimulationConfiguration simconf) {
        ...
        MapSimulationConfiguration simconfig = (MapSimulationConfiguration) simconf;
        ...
        massim.competition2015.GraphSimulationXMLStatisticsObserver.simulationName = simulationName + "_" + df.format(dt);
        ...
        massim.competition2015.GraphSimulationXMLStatisticsObserver.outputFolder = this.visualisationobserverOutputPath + System.getProperty("file.separator");
...
//        this.maxSizeY = simconfig.gridWidth * simconfig.cellWidth;
    ...
    protected synchronized void drawBackground(SimulationWorldState s_state) {
        ...
        MapSimulationWorldState state = (MapSimulationWorldState) s_state;

        this.output.create();
      //  this.output.drawGraph("graph", state.sizex, state.sizey, state.cityMap.getNodes(), state.cityMap.getAllEdges());
    ...
    protected synchronized void drawSimulation(int step, SimulationWorldState s_state, AgentState[] agentstates) {
        ...
        Map<String, Collection<massim.competition2015.scenario.GraphNode>> filteredNodes = filterNodes(state.cityMap.getNodes());
        Map<String, Collection<EdgeDummy>> filteredEdges = filterEdges(state.cityMap.getAllEdges(), state.cityMap.getNodes());
        ...
        for (String domTeam : filteredEdges.keySet()) {
            ...
            //draw zones but stop if it takes too long
            if(fastModeActivated == false) {
                boolean timedOut =
                        !this.output.drawZones(
                                filteredNodes.get(domTeam), filteredEdges.get(domTeam), zoneRenderingTimeoutMs, style);
                if(timedOut){
                    this.output.drawZonesFastMode(filteredNodes.get(domTeam), filteredEdges.get(domTeam), style);
                    fastModeActivated = true;
                    fastModeCounterInitial += fastModeCounterIncrease;
                    fastModeCounter = fastModeCounterInitial;
                }
            }
            else{
                this.output.drawZonesFastMode(filteredNodes.get(domTeam), filteredEdges.get(domTeam), style);
...
/*
        ...
        // B: Draw Agents
        Map<MapSimulationAgentState, Point> agPositions = new HashMap<>();
        Map<String, massim.competition2015.scenario.GraphNode> nodeMap = new HashMap<>();
        for (massim.competition2015.scenario.GraphNode n : state.cityMap.getNodes()) {
            nodeMap.put(n.name, n);
        }
        for (massim.competition2015.scenario.GraphNode n : state.cityMap.getNodes()) {

//            Vector<MapSimulationAgentState> ags = n.agents;
...
//            for (MapSimulationAgentState ag : ags) {
//
//                i++;
//
...
//            }// all agents for the current node drawn
        ...
        for (MapSimulationAgentState ag : state.agents) {
            ...
            if (action.equalsIgnoreCase("attack")) {
                ...
                for (MapSimulationAgentState ag2 : state.agents) {
...
//                        }
                ...
                String target = ag.lastActionParam;
                massim.competition2015.scenario.GraphNode nTarget = nodeMap.get(target);
...
//                }
    ...
    private Map<String, Collection<massim.competition2015.scenario.GraphNode>> filterNodes(Collection<massim.competition2015.scenario.GraphNode> nodes) {

        Map<String, Collection<massim.competition2015.scenario.GraphNode>> ret = new HashMap<>();

        for (massim.competition2015.scenario.GraphNode n : nodes) {

//            String domTeam = n.getDominatorTeam();
//
//            if (domTeam != null) {
//                Collection<massim.competition2015.scenario.GraphNode> coll = ret.get(domTeam);
//                if (coll == null) {
//                    coll = new Vector<>();
...
//            }
    ...
    private Map<String, Collection<EdgeDummy>> filterEdges(Collection<massim.competition2015.scenario.GraphEdge> edges, Collection<massim.competition2015.scenario.GraphNode> nodes) {

        Map<String, Collection<EdgeDummy>> ret = new HashMap<>();

        HashMap<massim.competition2015.scenario.GraphNode, Vector<EdgeDummy>> incoming = new HashMap<>();
        HashMap<massim.competition2015.scenario.GraphNode, Vector<EdgeDummy>> outgoing = new HashMap<>();
        for (massim.competition2015.scenario.GraphNode n : nodes) {
            incoming.put(n, new Vector<EdgeDummy>());
            outgoing.put(n, new Vector<EdgeDummy>());
        }

        for (massim.competition2015.scenario.GraphEdge e : edges) {

//            String domTeam = null;
//            if (e.getNode1().getDominatorTeam() != null && e.getNode1().getDominatorTeam() == e.getNode2().getDominatorTeam()) {
