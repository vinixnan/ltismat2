...
import massim.competition2015.scenario.GraphEdge;
import massim.competition2015.scenario.GraphNode;
...
public class GraphPolicy extends massim.visualization.svg.SvgXmlFile {
	...
	/* 
	 * TODO 2015: Statistics/visualization.
	 * This class is used to paint SVGs?
	 */
    ...
    private massim.competition2015.GraphSimulationVisualizationObserver vo;
    ...
    public GraphPolicy(massim.competition2015.GraphSimulationVisualizationObserver graphSimulationVisualizationObserver) {
     ...
     */
    public void drawGraph(String string, int sizeX, int sizeY, Collection<GraphNode> nodes, Collection<GraphEdge> edges) {
    ...
    public void drawEdge(GraphEdge e) {
        ...
        int width = LINE_WIDTH; //FIXME

        String classValue = "E";

        Element edge = this.doc.createElement("line");
        edge.setAttribute("x1", String.valueOf(e.getNode1().x));
        edge.setAttribute("y1", String.valueOf(e.getNode1().y));
        edge.setAttribute("x2", String.valueOf(e.getNode2().x));
        edge.setAttribute("y2", String.valueOf(e.getNode2().y));
    ...
    public void drawAgent(MapSimulationAgentState ag, Color agentColor, int agx, int agy) {
...
//        }
    ...
    /**
     * @return false iff zones were not drawn completely due to exceeded timeout
     */
    public boolean drawZones(Collection<GraphNode> nodesIn, Collection<EdgeDummy> edgesIn, long timeout, String style) {
    ...
    public void drawZonesFastMode(Collection<GraphNode> nodesIn, Collection<EdgeDummy> edgesIn, String style) {
        //Draw zones very fast (do not draw them at all, edges are already colored)
     ...
     */
    public int drawFirstTable(Collection<TeamState> teamsStates, long width, int currStep) {
        ...
        for (TeamState ts : teamsStates) {
            g.appendChild(createRow(false, y, width, ts.name, String.valueOf(ts.money), String.valueOf(ts.ranking), "", ""));
            y += 40;
        }
     ...
     */
    public int drawSecondTable(Collection<MapSimulationAgentState> agents, long width, int yValue) {
        ...
        for (MapSimulationAgentState ag : agents) {
            y = createDynamicRowColors(g, y, width, new Vector<>(Arrays.asList(ag.load, ag.role.loadCapacity)), new Vector<>(Arrays.asList(ag.getBatteryCharge(),ag.role.batteryCapacity)), 22, 33, new Vector<>(Arrays.asList(ag.lastAction,ag.lastActionParam,ag.lastActionResult)), ag, ag.lastActionResult.startsWith("failed"));
        }
    ...
    private void drawPath(LinkedList<EdgeDummy> pathOut, String style) {
        ...
        for (int a = 0; a < pathOut.size(); a++) {
            x[a] = (int)pathOut.get(a).node1.x;
            y[a] = (int)pathOut.get(a).node1.y;
        }
    ...
    private int createRow(Element g, int y, long width, String[] entries, MapSimulationAgentState ag) {
     ...
     * table
     */
    private int createStaticRow(Element g, int y, long width, String[] entries, MapSimulationAgentState ag) {
    ...
    private int createDynamicRowColors(Element g, int y, long width, Vector<Integer> energy, Vector<Integer> health, Integer strength, Integer visibility, Vector<String> Action, MapSimulationAgentState ag, boolean failed) {
        Element text;
        String anchor = "start";
    ...
    private int createDynamicRow(Element g, int y, long width, String[] entries, MapSimulationAgentState ag, boolean failed) {
    ...
    public void listAchievements(int yValue, long width, Collection<TeamState> teamsStates) {
        int y = yValue;
        y += 15;
...
//            }
    ...
    public void setTeamNames(Vector<String> teamNames2) {
        ...
        this.headInformationFirstLevel = "Graph Simulation 2015";
    ...
    private boolean isPolygonClockwise(LinkedList<EdgeDummy> path) {
        ...
        for (EdgeDummy edge : path) {
            points.add(new Point((int)edge.node1.x, (int)edge.node1.y));
        }
     ...
     */
    public void drawFixedInformation(massim.competition2015.MapSimulationWorldState state, long width) {
        ...
        List<MapSimulationAgentState> agents = state.agents;
    ...
    public void drawTargetLine(int x, int y, int x2, int y2, MapSimulationAgentState ag, String lastAction, String lastActionResult, Color agentColor) {
