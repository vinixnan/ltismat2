...
import massim.competition2014.scenario.DebugGraphGenerator;
import massim.competition2014.scenario.GraphEdge;
import massim.competition2014.scenario.GraphGenerator;
import massim.competition2014.scenario.GraphGenerator2013;
import massim.competition2014.scenario.GraphGeneratorTriangBalOpt;
import massim.competition2014.scenario.GraphGeneratorTriangulationBalanced;
...
/**
 * Holds the current state of a graph simulation (2014 Mars Scenario)
 */
public class GraphSimulationWorldState extends SimulationWorldState implements WorldState {
	...
	public GraphSimulationWorldState(GraphSimulationConfiguration config) {
		...
		// reflection
		try {
			Class generatorClass = Class.forName("massim.competition2014.scenario." + config.mapGenerator);
	...
	public void addAgent(GraphSimulationAgentState agent, Vector<Integer> agentPositions, boolean newPosition) {
		...
		} else if(this.generator instanceof DebugGraphGenerator){
			if(team1Name.equals("")){
				team1Name = agent.team;
			}
			int pos = 0;
			if (agent.team.equals(team1Name)){
				pos = team1Used;
				team1Used++;
			}
			else{
				pos = (nodes.size()-1)-team2Used;
				team2Used++;
			}
			node = this.nodes.get(pos);
		} else {
